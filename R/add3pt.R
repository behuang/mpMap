...
add3pt <- function(mpcross, newmpcross, newchr, mapfx=c("haldane", "kosambi"))
{
  ...
  for (ii in 1:length(mpcross$map))
  {
    ...
    if (nmrk>0) {
     ...
     rl <- mf(c(5, dmap/2, dmap[lmap-1]))
     rr <- mf(c(dmap[1], dmap/2, 5)) 

     if (mapfx=="kosambi")
	ro <- (rl+rr)/(1+4*rl*rr) else ro <- rl+rr-2*rl*rr

     #### note, we also need to compute the base lkhd to get LOD values
     rl <- c(rl, rep(.5, length(left)-1), mf(dmap[lmap-1]))
     rr <- c(rr, mf(dmap[1]), rep(.5, length(left)-1))
     ro <- c(ro, .5, mf(dmap), .5)
     left <- c(left, left)
     mid <- c(mid, mid)
     right <- c(right, right)
     ...
     ngen <- nai(mpcross$pedigree, mpcross$id)
     lk3pt <- .C("add3pt",  as.integer(finvec), as.integer(fouvec), as.integer(mpcross$pedigree[,1]), as.integer(mpcross$pedigree[,2]), as.integer(mpcross$pedigree[,3]), as.integer(nobs), as.integer(n.founders), as.integer(nmrk), as.integer(nrow(mpcross$pedigree)), as.integer(ngen), as.double(rl), as.double(rr), as.double(ro), as.integer(left), as.integer(mid), as.integer(right), as.integer(ncol(finchr)), as.integer(npos), out=integer(length=nmrk), lod=double(length=nmrk*npos/2), PACKAGE="mpMap")
