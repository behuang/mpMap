calcmpprob <- function(object, chr, step, mapfx, ibd, mrkpos)
{
	...
	for (ii in chr)
	{
		...
		ngen <- nai(object$pedigree, object$id)
		hap3pt <- .C("hap3ptfull",  as.integer(finvec), as.integer(fouvec), as.integer(object$pedigree[,1]), as.integer(object$pedigree[,2]), as.integer(object$pedigree[,3]), as.integer(n.finals), as.integer(n.founders), as.integer(lmap), as.integer(nrow(object$pedigree)), as.integer(ngen), out=double(length=n.pos*n.founders*n.finals), as.double(r12), as.double(r23), as.double(r13), as.integer(left), as.integer(mid), as.integer(right), as.integer(n.pos), PACKAGE="mpMap")
