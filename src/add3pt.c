...
void add3pt(int *finalg, int *founderg, int *id, int *mother, int *father, int *nfinals, int *nfounders, int *nmrk, int *nped, int* ngen, double *r12, double *r23, double *r13, int *left, int *mid, int *right, int *totmrk, int *npos, int *out, double *lod)
{
 ...
 hprob = (double*) R_alloc(20* *npos, sizeof(double));
 hproblod = (double*) R_alloc(20* *npos, sizeof(double));
 funnels = (int*) R_alloc(*nfounders * *nfinals, sizeof(int));
 ...
 cachedHclass = (int*)R_alloc(*nfounders * *nfounders * *nfounders, sizeof(int));
 ...
 for (k=0; k<*nfounders; k++)
 {
   for (l=0; l<*nfounders; l++)
   {
     for (m=0; m<*nfounders; m++)
     {
       ...
       hapi[1] = l;
       hapi[2] = m;
       cachedHclass[k * *nfounders * *nfounders + l * *nfounders + m] = hclass(hapi, *nfounders);
     }
   }
 }
 for (i=0; i<*nmrk; i++)
 ...
 for(j=0; j< *nfinals; j++)
 {
   pedfunnel(finalg[j*(*totmrk+1)], id, mother, father, funnels+j**nfounders, *nfounders, *nped, ngen[j]);
 }
 for (q=0; q<*nmrk; q++)
 {
   for (i=0; i<np; i++)
   ...
   for (j=0; j<*nfinals; j++) 
   {
    ...
    for (k=0; k<*nfounders; k++)
    {
 	cf1[k] = (fou[(funnels[j* *nfounders + k]-1)]==geni[0]);
	cf2[k] = (fou[*nfounders+(funnels[j * *nfounders + k]-1)]==geni[1]);
	cf3[k] = (fou[2*(*nfounders)+(funnels[j * *nfounders + k]-1)]==geni[2]);
	if (geni[0]<0) cf1[k] = 1;
	if (geni[1]<0) cf2[k] = 1;
  	if (geni[2]<0) cf3[k] = 1;
    }
   ...
   {
	hp += hprob[20*i + cachedHclass[k * *nfounders * *nfounders + l * *nfounders + m]];
